...
def plot_joint_vs_marginal_violin_by_interactivity_segment(config):
    df = load_df_from_pkl(config.fix_prob_df_by_interactivity_segment_path)
    categories = ["face", "out_of_roi"]
    df = df[df["fixation_category"].isin(categories)]
    df["joint"] = df["P(m1&m2)"]
    df["marginal"] = df["P(m1)*P(m2)"]
    ...
    agg_df = _compute_median_df(
        df, group_cols=["monkey_pair", "fixation_category", "interactivity", "segment_id"]
    )
    ...
    for i, interactivity in enumerate(interactivities):
        for j, category in enumerate(categories):
            ...
            melted = sub_agg_df.melt(
                id_vars=["monkey_pair", "segment_id"],
                value_vars=["P(m1)*P(m2)", "P(m1&m2)"],
                var_name="Probability Type",
                value_name="Probability"
            )
            ...
            # Aggregate per monkey_pair (across segments)
            mp_data = {
                row["monkey_pair"]: [row["P(m1)*P(m2)"], row["P(m1&m2)"]]
                for _, row in sub_agg_df.iterrows()
            }
            ...
            # Annotate using all segments (full df)
            stat_df = df[
                (df["fixation_category"] == category) &
                (df["interactivity"] == interactivity)
            ]
    ...
    plt.tight_layout(rect=[0, 0, 1, 0.93])
    _finalize_and_save(fig, config.plot_dir, "joint_fixation_probabilities_by_interactivity_segment.pdf")
